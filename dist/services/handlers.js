...
async function watermark({
  ...
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/watermark`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: form
  }, 45000);
...
async function pdf2img({
  ...
  validateFile(file, ".pdf");
  const form = buildFormData("file", file);
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/pdf2img`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: form
  }, 90000);
...
async function compresspdf({
  ...
  validateFile(file, ".pdf");
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/compresspdf`, {
    method: "POST",
    headers: getAuthHeaders(),
...
async function photo3x4({
  ...
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/photo3x4`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: form
  }, 30000);
...
async function summarize({
  ...
  return apiText("/api/summarize", {
    text: sanitizeText(text, 10000)
  });
}
...
async function pdfpages({
  ...
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/pdfpages`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: form
  }, 45000);
...
async function docxedit({
  ...
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/docxedit`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: form
  }, 30000);
...
async function cv({
  ...
  const response = await fetchWithTimeout(`${BACKEND_URL}/api/cv`, {
    method: "POST",
    headers: {
      ...getAuthHeaders(),
      "Content-Type": "application/json"
