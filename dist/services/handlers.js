...
async function pdftext({
  ...
  return {
    type: 'text',
    content: out.trim() || 'Matn topilmadi'
  };
}
async function pdflock({
  ...
  if (!password || !password.trim()) {
    return {
      type: 'text',
      content: '❌ Parolni kiriting (matn maydonida)'
    };
  }
...
async function pdf2img({
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    zip.file(`page_${i}.jpg`, dataUrl.split(',')[1], {
      base64: true
    });
  }
  const blob = await zip.generateAsync({
...
async function pdf2docx({
  ...
  if (!file) return {
    type: 'text',
    content: '❌ PDF faylini yuklang'
  };
  if (BACKEND_URL) {
    try {
      const form = new FormData();
      form.append('file', file);
      const r = await fetch(`${BACKEND_URL}/api/pdf2docx`, {
        method: 'POST',
      ...
      return {
        ...
        info: "Word formatiga o'tkazildi"
      };
    } catch (_) {
  ...
  return {
    ...
    info: 'Oddiy matn (DOCX uchun backend kerak)'
  };
}
async function docx2pdf({
  ...
  const form = new FormData();
  form.append('file', file);
  const r = await fetch(`${BACKEND_URL}/api/docx2pdf`, {
    method: 'POST',
    headers: getUserIdHeader(),
    body: form
  });
...
async function compresspdf({
  ...
  const info = saved > 0 ? `${saved}% kichiklashdi · ${srcPdf.numPages} sahifa · Matn rasm sifatida saqlanadi` : 'PDF allaqachon optimallashgan (matn-only fayllar kam siqiladi)';
  return {
    type: 'file',
    blob,
...
async function compresspptx({
  ...
  try {
    ...
    return {
      type: 'text',
      content: '❌ PPTX fayl o\'qilmadi. To\'g\'ri .pptx faylini yuklang.'
    };
  }
  ...
  for (const imgPath of mediaFiles) {
    ...
    const blobUrl = URL.createObjectURL(new Blob([imgBuf], {
      type: mimeType
    }));
    try {
  ...
  const saved = Math.round((1 - blob.size / file.size) * 100);
  const info = saved > 0 ? `${saved}% kichiklashdi · ${mediaFiles.length} ta rasm siqildi` : 'Allaqachon optimallashgan';
  return {
    type: 'file',
    blob,
    filename: 'compressed.pptx',
...
async function imgcompress({
  ...
  return new Promise(resolve => {
    canvas.toBlob(blob => {
      const saved = Math.round((1 - blob.size / file.size) * 100);
      const info = saved > 0 ? `${saved}% kichiklashdi` : 'Rasm allaqachon optimallashgan';
      resolve({
        type: 'file',
        blob,
...
function translit({
  ...
  while (i < src.length) {
    ...
    if (!matched) {
      const c = src[i];
      const mapped = LTR[c.toLowerCase()];
      if (mapped) {
...
function deadline({
  ...
  const m = text.match(/(\d{1,2})[.\-/](\d{1,2})[.\-/](\d{2,4})/) || text.match(/(\d{4})[.\-/](\d{1,2})[.\-/](\d{1,2})/);
  if (!m) return {
    type: 'text',
    content: '❌ Sanani kiriting: 31.12.2025 yoki 2025-12-31'
  };
  let d;
  if (m[1].length === 4) d = new Date(`${m[1]}-${m[2].padStart(2, '0')}-${m[3].padStart(2, '0')}`);else d = new Date(`${m[3].length === 2 ? '20' + m[3] : m[3]}-${m[2].padStart(2, '0')}-${m[1].padStart(2, '0')}`);
  const days = Math.ceil((d - Date.now()) / 86400000);
  if (isNaN(days)) return {
    type: 'text',
    content: '❌ Noto\'g\'ri sana formati'
  };
  const emoji = days < 0 ? '🔴' : days <= 3 ? '🚨' : days <= 7 ? '🟡' : '🟢';
  const msg = days < 0 ? `${Math.abs(days)} kun oldin o'tdi!` : days === 0 ? 'BUGUN!' : `${days} kun qoldi`;
  return {
    type: 'text',
    content: `📅 ${d.toLocaleDateString('uz-UZ')}\n${emoji} ${msg}`
  };
}
function stats({
  text
  ...
  if (!nums.length) return {
    type: 'text',
    content: '❌ Raqamlar kiriting. Masalan: 4 7 2 9 1 5'
  };
  const n = nums.length;
...
async function equation({
  ...
  try {
    ...
    return {
      type: 'text',
      content: `✅ ${text.trim()} = ${math.format(result, {
        precision: 10
      })}`
    };
  } catch (e) {
...
async function wiki({
  ...
  return {
    type: 'text',
    content: '❌ Wikipedia\'da topilmadi. Boshqa so\'z bilan qidiring.'
  };
}
async function books({
  ...
  try {
    ...
    if (!d.docs?.length) return {
      type: 'text',
      content: '📚 Kitob topilmadi'
    };
    const list = d.docs.map((b, i) => `${i + 1}. ${b.title}${b.author_name ? '\n   ✍️ ' + b.author_name[0] : ''}${b.first_publish_year ? '  📅 ' + b.first_publish_year : ''}`).join('\n\n');
    return {
      type: 'text',
      content: `📚 Natijalar (${d.numFound} ta topildi):\n\n${list}`
    };
  } catch {
    return {
      type: 'text',
      content: '❌ Kitob qidirishda xatolik. Internet aloqasini tekshiring.'
    };
  }
}
...
function cert({
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(W / 2 - 180, 345);
  ctx.lineTo(W / 2 + 180, 345);
  ctx.stroke();
  ctx.fillStyle = 'rgba(255,255,255,0.4)';
  ctx.font = '14px sans-serif';
  ctx.fillText(new Date().toLocaleDateString('uz-UZ', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  }), W / 2, 480);
