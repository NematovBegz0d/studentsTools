...
function MergePdfPro({ t, accent, onToast }) {
  ...
  // ─── DONE ─────────────────────────────────────────────────────────
  // Layout: natija vertikal o'rtada, tugmalar pastda — DocxEditPro bilan
  // bir xil ko'rinish (UI consistency).
  if (step === "done" && result) {
    return (
      <div
        style={{
      ...
      >
        ...
        >
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              type="button"
              onClick={() => _mpDownload(result.blob, result.filename)}
              style={{
                flex: 1,
                padding: "13px 14px",
                borderRadius: 14,
                ...
                fontWeight: 700,
                cursor: "pointer",
                boxShadow: `0 6px 18px ${(accent || MP_ACCENT) + "40"}`,
              }}
            >
              Yuklab olish
            </button>
          ...
          <button
            type="button"
            onClick={() => _mpSendToBot(result.blob, result.filename, onToast)}
            style={{
              width: "100%",
              padding: "12px 14px",
              borderRadius: 14,
