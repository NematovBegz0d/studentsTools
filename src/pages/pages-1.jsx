...
function HomePage({ t, accent, cardStyle, onOpenService, onGoTo, user }) {
  ...
  return (
    <div style={{ paddingBottom: 110 }}>
      {/* Greeting */}
      <div style={{ padding: '24px 18px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <Avatar name={fullName} size={48} ring accent={accent} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 500 }}>
            {t.hello},
          </div>
          <div style={{ color: 'var(--text-primary)', fontSize: 20, fontWeight: 700, letterSpacing: -0.4 }}>
            {firstName} 👋
          </div>
        </div>
        {/* notif bell */}
        <button className="press" type="button" style={{
          ...
          <div style={{
            ...
            background: '#ef4444', border: '1.5px solid var(--bg-solid)',
          }} />
        </button>
      </div>
      ...
      <div style={{ padding: '4px 18px 18px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {stats.map((s, i) => (
          <div key={i} style={{
            ...
            <div style={{ color: 'var(--text-muted)', fontSize: 10, marginTop: 2, fontWeight: 500 }}>
              {s.label}
            </div>
          </div>
        ))}
      ...
      <div style={{ padding: '0 18px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <h2 style={{ margin: 0, color: 'var(--text-primary)', fontSize: 17, fontWeight: 700, letterSpacing: -0.2 }}>
          {t.quickAccess}
        </h2>
      ...
      <div style={{ padding: '0 18px 22px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
        {quickItems.map(item => (
          <button
            key={item.id}
            className="press"
            ...
            style={{
              ...
              borderRadius: 16, cursor: 'pointer', font: 'inherit',
          ...
          >
            ...
            <div style={{
              fontSize: 10.5, color: 'var(--text-muted)', lineHeight: 1.35,
        ...
        {t.activities.map((a, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ width: 36, height: 36, borderRadius: 10, background: bg, border: border, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Icon name={svc.id} size={18} color={catColor} strokeWidth={1.8} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: 'var(--text-primary)', fontSize: 13.5, fontWeight: 600 }}>{meta.name}</div>
                <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 1 }}>{a.time}</div>
              </div>
              <div style={{ width: 20, height: 20, borderRadius: 99, background: 'rgba(34,197,94,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function FreePage({ t, accent, cardStyle, onOpenService }) {
  ...
  return (
    <div style={{ paddingBottom: 110 }}>
      ...
      <div style={{ padding: '4px 18px 10px' }}>
        <div style={{
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M21 21l-4.3-4.3" stroke="var(--text-muted)" strokeWidth="2" strokeLinecap="round"/>
          </svg>
          <input
            value={q}
            ...
            style={{
              ...
              color: 'var(--text-primary)', fontSize: 14, outline: 'none', font: 'inherit',
            }}
          />
          {q && (
            <button onClick={() => setQ('')} style={{
              ...
              <svg width="9" height="9" viewBox="0 0 24 24" fill="none">
                <path d="M6 6l12 12M18 6L6 18" stroke="var(--text-secondary)" strokeWidth="2.5" strokeLinecap="round"/>
              </svg>
            </button>
          )}
        </div>
      ...
      {/* Categories */}
      <div style={{ padding: '4px 0 14px', overflowX: 'auto', overflowY: 'hidden', scrollbarWidth: 'none' }} className="hide-scroll">
        <div style={{ display: 'flex', gap: 7, padding: '0 18px', width: 'max-content' }}>
          {CATEGORIES.map(c => {
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: '60px 30px', textAlign: 'center' }}>
          ...
          <div style={{ color: 'var(--text-primary)', fontSize: 16, fontWeight: 600 }}>{t.noResults}</div>
          <div style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>{t.noResultsSub}</div>
        </div>
      ) : (
          ...
          return (
            ...
            >
              <div style={{
                ...
                background: 'rgba(245,158,11,0.16)', border: '0.5px solid rgba(245,158,11,0.30)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
              }}>
              ...
              {!disabled && !isSubscribed && (
                <div style={{
                  ...
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                    ...
                    <path d="M8 11V8a4 4 0 1 1 8 0v3" stroke="var(--text-secondary)" strokeWidth="2" strokeLinecap="round"/>
                  </svg>
                </div>
              )}
            </button>
        ...
        <div style={{ padding: '24px 18px 0' }}>
          <div style={{
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ fontSize: 17, fontWeight: 800, marginBottom: 4, letterSpacing: -0.3 }}>{t.bannerTitle}</div>
              <div style={{ fontSize: 12.5, opacity: 0.92, marginBottom: 14, textWrap: 'pretty' }}>{t.bannerSub}</div>
              <button onClick={() => onGoTo('plans')} className="press" style={{
